...
def process_single_image(input_image_path, text_prompts, root_folder, output_folder):
    for text_prompt, box_threshold in text_prompts.items():
        relative_path = os.path.relpath(os.path.dirname(input_image_path), root_folder)
        output_subfolder = os.path.join(output_folder, relative_path)
        try:
            # Create boxes and extract images
            boxes_xyxy, annotated_frame = createBoxes(input_image_path, text_prompt, box_threshold)
...
def process_images(root_folder, output_folder, start_from_zero=True):
    ...
    total_images = len(image_paths) * len(text_prompts)

    with tqdm(total=total_images, desc="Processing Images") as pbar:
        with Pool(processes=cpu_count()) as pool:
            for _ in pool.imap_unordered(partial(process_single_image, text_prompts=text_prompts, root_folder=root_folder, output_folder=output_folder), image_paths):
                pbar.update(1)
