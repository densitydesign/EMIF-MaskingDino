...
def createBoxes(image_path, text_prompt, token_spans=None, ):
...
def extractImages(boxes_xyxy, image_path, text_prompts,
                  output_folder,
                  bypass_filling = False):

    sam = create_sam_model(name="xl1",
                            weight_url="efficientvit/xl1.pt")
    ...
    if boxes_xyxy.size == 0:
        ...
        prompt_word = next((word for word in text_prompts.split() if len(word) > 3), "prompt")
    ...
    bw_mask_output_path = os.path.join(image_output_folder, f"2_{base_filename}_{prompt_word}_mask.png")
                ...
                for file in files:
                    if file.lower().endswith(('.png', '.jpg', '.jpeg')):
                        ...
                        try:
                            ...
                            boxes_xyxy, annotated_frame = createBoxes(input_image_path, text_prompt)
