#STABLE version was updated on 30.05 at the end, 
#works, still bugs of view
...
def extractImages(boxes_xyxy, image_path, output_folder, should_refine=False):
    ...
    masks_refined, _, _ = predictor.predict_torch(
        ...
        mask_input=None,  # Use low-res masks from the first prediction
    ...
    combined_mask = np.any(masks_refined, axis=0)
    ...
    for c in range(3):
        cutout[:, :, c] = np.where(combined_mask, image[:, :, c], 0)
    ...
    final_output_path = os.path.join(image_output_folder, f"{base_filename}_{prompt_word}_1.png")
...
root_folder = "/Users/tommasoprinetti/Documents/DENSITY_OFFICE/EMIF/COMPRESSED_FINAL_IMAGES/Greece"
